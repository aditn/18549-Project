...
// MultiDataStore depends on date changes, so we listen to DateStore
const MultiDataStore = Reflux.createStore({
  data: {
    general: [],
    previous: []
  },
  ...
  onAggreChange(newAggre){
    ...
    function get(g_dtd, p_dtd, callback) {
      ...
      API_wrap.getMultiData(from,to,dimension,filter,
        (data)=>{
          self.data.general = data;
          g_dtd.resolve();
        });

      API_wrap.getMultiData(
