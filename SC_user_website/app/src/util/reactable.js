(function (root, factory) {
        ...
        render() {
            ...
            if (
                ...
                children = children.concat(this.props.columns.map(function(column, i) {
                  ...
                  // console.log(this.props.data.hasOwnProperty(column.key));
    ...
    var FiltererInput = React.createClass({
        onChange() {
          this.props.onFilter(this.getDOMNode().children[0].value);
        },
        render() {
            return (
    ...
    /**
    * ____                   _ _
    * |  _ \ __ _  __ _ _ __ (_) |_ ___  _ __
    * | |_) / _` |/ _` | '_ \| | __/ _ \| '__|
    * |  __/ (_| | (_| | | | | | || (_) | |
    * |_|   \__,_|\__, |_| |_|_|\__\___/|_|
    *            |___/
    */
    ...
    */
    var Table = exports.Table = React.createClass({
        ...
        render() {
            ...
            if (columns.length > 0) {
                ...
                // This translates columns from array of string to
                // array of objects that contain key and label
                columns = this.translateColumnsArray(columns);
            }

            // Build up table rows based on given data
            if (this.data && typeof this.data.map === 'function') {
                // Build up the columns array
                // Concat simply append, so actually this line does nothing more
                // than assigning children to the result of this.data.map(fn)
            ...
            // Determine if we need to fix certain columns
            if (
              this.props.fixedColumns &&
                Array.isArray(this.props.fixedColumns) &&     this.props.fixedColumns.length > 0)
           {
             // Fix certain columns!
             // For this we assume fixedColumns contains keys of columns that need fixing
             var fixedHead = findMatches(this.props.fixedColumns,columns,true);
             var scrollHead = findMatches(this.props.fixedColumns,columns,false);

             var fixedBody = findFixedCol(fixedHead, currentChildren, this);
             var scrollBody = findFixedCol(scrollHead,currentChildren, this);

             return(
               <div className="dual-table">
                 <div className="pinned">
                   <table className="sortable ui small basic table">
                     <Thead columns={fixedHead}
                       filtering={filtering} onFilter={this.filterBy}
             ...
             // No fixed columns, return what reactable used to be
             return <table {...props}>{[
    ...
    function findMatches(listOfString,listOfObject,add){
      var result = [];
      for(var i = 0, l = listOfObject.length-1; i < l; i++){
        var cur = listOfObject[i];
        if(cur == undefined || cur.key == undefined){
          continue;
        }
        var index = listOfString.indexOf(cur.key);
        if(add){
          // We find it and push
          if(index !== -1){
            // Found it
    ...
    function findFixedCol(listOfObject,listOfReactElement,self){
      // Gotta retrun <Tr/> elements
      var result = [];
      var listOfString = listOfObject.map(function(elem,i){
      ...
      for(var i = 0, l = listOfReactElement.length; i < l; i++){
        var rElem = listOfReactElement[i];
        var data = rElem.props.data;
        var restructuredData = {};

        for (var key in data) {
          var index = listOfString.indexOf(key);
          if(index !== -1){
            // We need to push this element
            restructuredData[key] = data[key]
          }
        }
        result.push(
          <Tr columns={listOfObject} key={i} data={restructuredData} children={self.children}/>
        )
      }
      return result;
    }
